...
const DetailsPage = () => {
  ...
  const handleGetData = async () => {
    ...
    try {
      ...
      if (data?.error) {
        Alert.alert(data?.error?.info);
        return;
      }
      const { temperature, wind_speed, weather_icons, precip } =
        data?.current as IWeatherData;
      const wData = { temperature, wind_speed, weather_icons, precip };
      const cData = [];
      for (let key in wData) {
        const keyT = key as TKeyWeatherData;
        let value =
          typeof wData[keyT] === "number"
            ? wData[keyT].toString()
            : wData[keyT];
        let isFlag = false;
    ...
    <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
      ...
      >
        <View style={{ marginVertical: 16 }}>
          {processDataCountryData?.map((data, index) => (
            <RowDataItem key={index} {...data} />
          ))}
        </View>
        <View style={{ marginVertical: 16 }}>
          <TouchableOpacity
